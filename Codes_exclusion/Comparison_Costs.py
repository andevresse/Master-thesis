...
def compare_costs(file_ref,filename) :
    f = open(r'Ref\cost_breakdown.txt', "r")
    lines = f.readlines()
    tech = [None]*128
    ...
    df_ref = pd.read_csv(filename_ref,sep = "\t",header = None,skiprows = [0],index_col = 0)

    try :
        df_sub = pd.read_csv(filename_sub,sep = "\t",header = None,skiprows = [0],index_col = 0)
        ...
        for i in range (0,128) :
            for k in range (1,4) :
                cost_ref_tech[i] +=df_ref[k][i]
                cost_sub_tech[i] +=df_sub[k][i]
            
            if cost_ref_tech[i] - cost_sub_tech[i] != 0 :
                output = open(r'Comparison_file_cost.txt','a')
